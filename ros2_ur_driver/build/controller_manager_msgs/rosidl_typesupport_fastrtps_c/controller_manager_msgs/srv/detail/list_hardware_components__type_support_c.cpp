...
{
  return max_serialized_size_controller_manager_msgs__srv__ListHardwareComponents_Request(
    full_bounded, 0);
}
  ...
  void * untyped_ros_message)
{
  ...
  // Field name: component
  {
    ...
    if (!controller_manager_msgs__msg__HardwareComponentState__Sequence__init(&ros_message->component, size)) {
      return "failed to create array for field 'component'";
    }
    auto array_ptr = ros_message->component.data;
    for (size_t i = 0; i < size; ++i) {
...
{
  return max_serialized_size_controller_manager_msgs__srv__ListHardwareComponents_Response(
    full_bounded, 0);
}
